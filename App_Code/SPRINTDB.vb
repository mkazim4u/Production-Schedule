...
Public Class SprintDB
    ...
    Private Shared Sub CheckConnectionStringInitialised()
        If String.IsNullOrEmpty(_sConn) Then
            WebMsgBox.Show("SPRINTDB.CheckConnectionStringInitialised: Connection string has not been initialised!")
            Throw New Exception("SPRINTDB.CheckConnectionStringInitialised: Connection string has not been initialised!")
        End If
    End Sub
    ...
    Public Shared Function Query(ByVal sQuery As String) As DataTable
        ...
        Catch ex As Exception
            WebMsgBox.Show("Error in SPRINTDB.ExecuteQueryToDataTable executing: " & sQuery & " : " & ex.Message)
        Finally
            oConn.Close()
        End Try
        Query = oDataTable
    ...
    Public Shared Function Query(ByVal sQuery As String, ByVal sTextFieldName As String, ByVal sValueFieldName As String) As ListItemCollection
        ...
        Try
            ...
            End If
        Catch ex As Exception
            WebMsgBox.Show("Error in SPRINTDB.ExecuteQueryToListItemCollection: " & ex.Message)
        Finally
            oConn.Close()
        End Try
